...
int sql_parse(string &s)
{
  ...
  if (_select.take(s, i))
  {
    ...
    if (j == 1){
		find_index();
		if(index_lookup[search_list.begin()->first])l = search();
		else j=2;
	}
#endif
    {
  ...
  else if (_delete.take(s, i))
  {
    ...
    if (!(x1 && x2))
      return 0;
    string &a = table_list.back();
    int j = tables[a].table_address.block_number;
    list<recordData> l = rm.selectRecord(FILENAME, j);
    int tot = 0;
    auto st = clock();
    for (auto it = l.begin(); it != l.end(); it++)
  ...
  else if (_create.take(s, i))
  {
    if (_table.take(s, i))
    {
      ...
      do
      {
        ...
        if (_primary_key.take(s, i))
        {
          ...
          if (s[i++] != ')')
            return 0;
          // TODO: commit changes
          // insert(tables[new_table].table_address,p);
          CREATEINDEX(Pi.back());
#ifdef _INDEX_MANAGER_ENABLE
          block *m = buffermanager.getNewBlockBufferUnused(INDEX_FILE.data());
          before_Head_Block_Insert(m->blocknum);
          tables[new_table].primary_key = Pi;
          tables[new_table].index["0"] = {{FILENAME, m->blocknum, 0}, Pi};
#endif
        }
        else
        {
      ...
      tables[new_table].table_address = {FILENAME, j, 0};
      return 1;
    }
    else if (_index.take(s, i))
    {
      ...
      i = skip(s, i + 1);
      readlist(s, i);
	  if(tables[new_table].collum.count(Pi.back())==0)return 0;
      find_attributes();
      if (s[skip(s, i + 1)] != ';')
        return 0;
	  if(tables[new_table].collum[Pi.back()].unique==0){
		cout<<endl<<"ERROR: cannot index on non-unique attributes"<<endl;
	  }
#ifdef _INDEX_MANAGER_ENABLE
      CREATEINDEX(Pi);
      block *memory_storage = buffermanager.getNewBlockBufferUnused(INDEX_FILE.data());
      int index_header = memory_storage->blocknum;
      tables[new_table].index[new_attribute] = {{INDEX_FILE, index_header, 0},
                                                Pi};
  ...
  else if (_insert_into.take(s, i))
  {
	...
	if(inform.blocknum_offset){
		cout<<endl<<"repetition in primary key"<<endl;
		return 0;
        }
...
int find_index(){
	...
	for(auto it=search_list.begin();it!=search_list.end();it++){
		...
		for(;jt!=t.index.end();jt++)if(jt->second.indexed_attributes[0]==it->first){
