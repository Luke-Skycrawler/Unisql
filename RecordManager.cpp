...
recordData *RecordManager::insertRecord(char *filename, const int blocknum,
  ...
  if (isexec == true) {
    if (curexecblock == 0) {
      curexecblock = blocknum;
    } else {
      curblocknum = curexecblock;
      useblock = bufmanager.getBlock(filename, curexecblock);
    }
  }
  do {
    ...
    if (freelist + 4 + recordsize <= BLOCK_SIZE) {
      ...
      curexecblock = curblocknum;  // update the current block
      break;
    } else if (nextblock) {
  ...
  if (isinsert == false) {
    ...
    curexecblock = newblock->blocknum;
  }
  return newrecord;
}
...
void RecordManager::deleteRecord(char *filename, const int headblocknum,
  ...
  char *datafield = headerblock->data;
  int recordnum, freelist, total_recordnum, recordsize, recordlist;
  int nextrecord;
  memcpy(&total_recordnum, datafield + 16, 4);
  memcpy(&recordsize, datafield + 20, 4);
  // char *recordcontent = new char[recordsize];
  // update the total_recordnum
  total_recordnum -= 1;
  memcpy(datafield + 16, &total_recordnum, 4);
  bufmanager.setDirty(headerblock);
  // find the block to be altered
  ...
  memcpy(datafield + currecord, &nextrecord, 4);  // connect to the nextrecord
  ...
  else
  {
      ...
      memcpy(datafield + record.offset - 4,datafield + nextrecord,recordsize +
  4);
  }
  */
...
void RecordManager::startExecMode() { isexec = true; }

void RecordManager::shutExecMode() {
  isexec = false;
  curexecblock = 0;
}
/*
