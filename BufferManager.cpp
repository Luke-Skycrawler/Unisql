...
int BufferManager::mallocBlock(const char *filename)
{
    FILE *fp = fopen(filename,"rb+");
    if(fp == NULL)
    {
...
block *BufferManager::getNewBlockBufferUnused(const char *filename)
{
...
blocknode *BufferManager::getNewBlock(char *filename)
{
    ...
    return getBlock(filename,blocknum); //read the data from disk and add the block into buffer
}
...
blocknode *BufferManager::getBlock(char * filename, int blocknum)
{
    ...
    if(iter == mapBuffer.end()) //can not find this block in the buffer and read from disk
    {
        FILE *fp = fopen(filename,"rb");
        if(fp == NULL)
        {
        ...
        if(bufblocknum < BUFFER_SIZE)
        {
            /*
            int i;
            for(i = 0; i < BUFFER_SIZE; ++i)    //linear search, better: binary search, best: freelist
                if(buf[i] == NULL)
                    break;
            */
            if(freebuf.empty())
        ...
        {
            blocknode *tailblock = headblock -> preblock; //double circular linked list
            ...
            {
                writeBlockBacktoDisk(filename, tempblock);  //convert (blocknode *) to (block *)
            }
            {
                delete tempblock->data;
                delete tempblock;
...
block *BufferManager::getBlockBufferUnused(char *filename, int blocknum)
{
    ...
    if(fp == NULL)
    {
        std::cout << filename << " open failed when getBlockBufferUnused" << std::endl;
        exit(1);
    }
...
void BufferManager::showBufBlockList()
{
    ...
    {
        if(head!=NULL)
            std::cout << head->blocknum << std::endl;
        head = head->nextblock;
        if(head == headblock)break;
    }
}

void BufferManager::setDirty(blocknode *useblock)
{
    useblock->dirty = true;
}
/*
